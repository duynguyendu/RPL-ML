...
def convert_to_c_linear(
    ...
    terms = "".join(f" + {float(coef[i])!r} * (double){name}" for i, name in enumerate(feature_names))
    body_lines = [
        f"uint16_t {func_name}({_c_params(feature_names)}) {{",
        f"    double result = {intercept!r}{terms};",
...
"""
    source = (
        "/* Auto-generated by scripts/models/to_c.py (linear) from a "
        "trained model.\n"
        " * DO NOT EDIT BY HAND -- retrain and re-run the converter "
        "instead. */\n"
        f'#include "{func_name}.h"\n\n' + source_body + "\n"
