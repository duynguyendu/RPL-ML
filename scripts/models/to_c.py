...
_TYPE_MAX_ABS = {"uint8_t": 255, "uint16_t": 65535, "int16_t": 32768}
_INT32_BUDGET = 0.9 * 2**31  # headroom under int32_t's ~2.147e9 ceiling


def _pick_linear_scale(coef, intercept: float, feature_names: list[str]) -> tuple[int, bool]:
    worst_case = abs(intercept) + sum(
        abs(float(coef[i])) * _TYPE_MAX_ABS.get(FEATURE_C_TYPES.get(name, "uint16_t"), 65535)
        for i, name in enumerate(feature_names)
    )
    if worst_case <= 0:
        return 10000, False

    max_scale = _INT32_BUDGET / worst_case
    if max_scale < 1:
        return 1, True  # even unscaled coefficients risk overflowing int32_t

    scale = 1
    while scale * 10 <= max_scale:
        scale *= 10
    return scale, False


def _linear_fixed_params(model) -> tuple[list[str], list[int], int, int, bool]:
    feature_names = [str(name) for name in model.feature_names_in_]
    coef = model.coef_
    intercept = model.intercept_
    intercept = float(intercept if np.isscalar(intercept) else intercept[0])

    scale, needs_wide = _pick_linear_scale(coef, intercept, feature_names)
    coef_fixed = [round(float(c) * scale) for c in coef]
    intercept_fixed = round(intercept * scale)
    return feature_names, coef_fixed, intercept_fixed, scale, needs_wide


def _linear_predict_fixed(
    coef_fixed: list[int], intercept_fixed: int, scale: int, rows: np.ndarray
) -> np.ndarray:
    sums = intercept_fixed + rows.astype(np.int64) @ np.array(coef_fixed, dtype=np.int64)
    results = np.where(sums >= 0, sums // scale, -(-sums // scale))
...
def convert_to_c_linear(
    ...
    model = joblib.load(model_path)
    model_type = type(model).__name__
    if model_type not in LINEAR_SUPPORTED_ESTIMATORS:
        return None

    feature_names, coef_fixed, intercept_fixed, scale, needs_wide = _linear_fixed_params(model)
    acc_type = "int64_t" if needs_wide else "int32_t"

    body_lines = [
        f"uint16_t {func_name}({_c_params(feature_names)}) {{",
        f"    {acc_type} sum = {intercept_fixed};",
        *(
            f"    sum += {cf} * ({acc_type}){name};"
            for cf, name in zip(coef_fixed, feature_names)
        ),
        f"    {acc_type} result = sum / {scale};",
...
def verify_linear(model_path: str | Path, data_dir: str | Path) -> float | None:
    model = joblib.load(model_path)
    if type(model).__name__ not in LINEAR_SUPPORTED_ESTIMATORS:
        return None

    feature_names, coef_fixed, intercept_fixed, scale, _ = _linear_fixed_params(model)
    df = pd.read_csv(Path(data_dir) / "train.csv")
    df = df.dropna(subset=feature_names)
    rows = df[feature_names].to_numpy()

    float_preds = model.predict(df[feature_names])
    fixed_preds = _linear_predict_fixed(coef_fixed, intercept_fixed, scale, rows)

    mae = float(np.abs(fixed_preds - float_preds).mean())
    print(
        f"[linear] quantization error vs float model: MAE={mae:.2f} "
        f"(pdr scale 0-65535, scale={scale}, over {len(df)} rows)"
    )
    return mae
