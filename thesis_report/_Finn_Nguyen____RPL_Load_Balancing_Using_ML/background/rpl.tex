...
% TODO: include image of the layers in RPL
...
Figure~\ref{fig:dodag-example}. Within this
...
translates one or more metrics into rank. The RPL
...
\end{equation}
where $\mathit{Rank}(N)$ is the resulting rank of the node under consideration,
$\mathit{Rank}(P)$ is the rank already advertised by the candidate parent $P$,
and $\mathit{Rank\_Increase}$ is a positive value determined by the OF,
...
$\mathit{Rank\_Increase}$ and how parent is selected depends on the OF in use,
...
and the maximum sending interval time
...
$[I_{\mathrm{min}}, I_{\mathrm{max\_time}}]$, usually the initial interval is set to $I = I_{\mathrm{min}}$
...
to $I_{\mathrm{max\_time}}$ i.e. $I = \min(I \times 2, I_{\mathrm{max\_time}})$

Upon detecting inconsistent messages, The interval $I$ is reset to
$I_{\mathrm{min}}$ to increase the DIO sending rate.
...
properties of a certain neighbour, which compute $\mathit{Rank\_Increase}$ as
\begin{equation}
    \mathit{Rank\_Increase} = (\mathit{Rf} \times \mathit{Sp} + \mathit{Sr}) \times \mathit{MinHopRankIncrease}
\end{equation}
...
We can select
OF0 parameters by
selecting $\mathit{Rf} = 1$, $\mathit{Sp} = 1$ and $\mathit{Sr} = 0$, which reduces
$\mathit{Rank\_Increase}$ to a simple fixed per-hop increment of 256. However,
